...
export function PackCard({ name, onClick, disabled }: PackCardProps) {
  ...
  return (
    <motion.button
      ...
      className="group gradient-pack w-full rounded-2xl p-4 text-left shadow-lg
                 hover:shadow-2xl
                 disabled:opacity-50 disabled:cursor-not-allowed
                 animate-wiggle-hover"
    >
      <div className="flex items-center gap-3">
        <span className="text-3xl md:text-4xl transition-all emoji-hover-bounce">{emoji}</span>
