export interface NewsArticle {
  id: string;
  emoji: string;
  title: string;
  author: string;
  date: string;
  category: string;
  content: string;
}

export const newsArticles: NewsArticle[] = [
  {
    id: "1",
    emoji: "🏀",
    title: "LeBron James Retires from NBA to Focus on Arklet Full-Time",
    author: "ESPN Blook Division",
    date: "Feb 7, 2026",
    category: "Celebrity",
    content:
      "In a stunning press conference today, LeBron James announced his retirement from professional basketball. \"I've been chasing Celestials for months now and honestly, it's more competitive than the NBA,\" James told reporters while nervously opening packs on his phone under the podium. His agent declined to comment but was spotted trading a Mythic in the parking lot.",
  },
  {
    id: "2",
    emoji: "🎤",
    title: "Taylor Swift Drops Surprise Album 'The Trades Department' About Lost Arklet Deals",
    author: "Rolling Stone Gaming",
    date: "Feb 6, 2026",
    category: "Celebrity",
    content:
      "Taylor Swift's 14th studio album features 16 tracks entirely about Arklet trading gone wrong. Lead single 'We Are Never Getting That Blook Back' has already topped charts in 47 countries. Track 7, 'All Too Well (10 Minute Version) (From the Pack Opening Vault),' is a heart-wrenching ballad about accidentally trading away a Divine.",
  },
  {
    id: "3",
    emoji: "💪",
    title: "The Rock Reveals He Opens 500 Packs Before Every Workout",
    author: "Muscle & Blooks Magazine",
    date: "Feb 5, 2026",
    category: "Celebrity",
    content:
      "Dwayne 'The Rock' Johnson shared his morning routine on Instagram: 4:00 AM wake up, 4:01 AM open Arklet packs for 2 hours, 6:01 AM gym. \"The pack-opening energy carries into my deadlifts,\" Johnson explained, flexing while revealing he has 47 Legendaries. \"Can you smell what The Rock is pulling?\"",
  },
  {
    id: "4",
    emoji: "🎁",
    title: "MrBeast Gives Away 10,000 Divine Blooks in Latest Video",
    author: "YouTube Gaming Weekly",
    date: "Feb 4, 2026",
    category: "Celebrity",
    content:
      "In his latest video titled 'I Gave Away $10,000,000 Worth of Blooks,' MrBeast hosted the largest Arklet giveaway in history. Contestants had to survive 24 hours in a room filled entirely with printed-out Common Blooks to win. The video has already garnered 200 million views. Chandler somehow lost all his winnings within an hour.",
  },
  {
    id: "5",
    emoji: "🚀",
    title: "Elon Musk Proposes 'ArkletCoin' Cryptocurrency Backed by Blook Rarity",
    author: "TechCrunch Blooks",
    date: "Feb 3, 2026",
    category: "Tech",
    content:
      "In a series of late-night tweets, Elon Musk proposed a new cryptocurrency where each coin's value is determined by Blook rarity. \"One Divine = one Bitcoin, it's simple math,\" Musk wrote, followed by 14 rocket emojis. The SEC has already begun an investigation. Dogecoin dropped 12% in response.",
  },
  {
    id: "6",
    emoji: "🐉",
    title: "BREAKING: xXDragon_SlayerXx Pulls 3 Divines in a Single Day",
    author: "Arklet Times",
    date: "Feb 7, 2026",
    category: "Player",
    content:
      "Local legend xXDragon_SlayerXx has reportedly pulled three Divine-rarity Blooks within a 6-hour session, shattering all known records. Scientists at MIT are investigating whether this violates the laws of probability. \"I'm just built different,\" they said in an exclusive interview conducted entirely through in-game chat. Their friends remain skeptical.",
  },
  {
    id: "7",
    emoji: "🤦",
    title: "noobmaster69 Accidentally Trades Entire Collection for a Single Common",
    author: "Arklet Tribune",
    date: "Feb 6, 2026",
    category: "Player",
    content:
      "In what experts are calling 'the worst trade in Arklet history,' user noobmaster69 traded their complete collection of 2,847 Blooks — including 3 Celestials — for a single Common-rarity Blook they 'thought looked cool.' When reached for comment, noobmaster69 said: \"Wait, you can't undo trades?\" A moment of silence was held in the global chat.",
  },
  {
    id: "8",
    emoji: "👶",
    title: "8-Year-Old Becomes Youngest Player to Complete Full Arklet Collection",
    author: "Arklet Gazette",
    date: "Feb 5, 2026",
    category: "Player",
    content:
      "Timmy 'lil_packmaster' Rodriguez, age 8, has become the youngest player to collect every single Blook in Arklet, including all Divine-rarity items. When asked about his strategy, Timmy said: \"I just click the button a lot.\" His parents report he has not done homework in three weeks but are 'cautiously proud.' Harvard has reportedly reached out with a scholarship offer.",
  },
  {
    id: "9",
    emoji: "🎬",
    title: "Netflix Announces 'Blook Wars' Reality Show — 16 Players, One Divine",
    author: "Entertainment Weekly",
    date: "Feb 4, 2026",
    category: "Entertainment",
    content:
      "Netflix has greenlit a 10-episode reality competition series where 16 Arklet players compete in pack-opening challenges, trading battles, and multiplayer showdowns. The grand prize: a guaranteed Divine pull. Host Gordon Ramsay was overheard yelling 'This Blook is RAW!' during filming. Premiere date set for March 2026.",
  },
  {
    id: "10",
    emoji: "🏫",
    title: "Schools Report 400% Increase in 'Bathroom Breaks' Since Arklet Launch",
    author: "The Onion (but real)",
    date: "Feb 3, 2026",
    category: "Trending",
    content:
      "A nationwide survey of middle schools has found that bathroom break requests have skyrocketed by 400% since Arklet's launch, with the average break lasting exactly the time needed to open one pack. Teachers report hearing triumphant screaming from bathroom stalls. One principal installed pack-opening sound detection software, which was immediately defeated by students switching to vibrate mode.",
  },
];
